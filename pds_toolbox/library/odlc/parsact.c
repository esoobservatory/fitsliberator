...

{
  ...
  if (new_units == NULL)
	...
	case TV_NULL_VALUE:
	    current_units = ODLcurrent_value->item.value.null_data.units;
            if (current_units != NULL)
            {
                break;
            }
           
            /* This is the first units field for a null value */
            ODLcurrent_value->item.value.null_data.units = new_units;
            return;
  ...
  if (exponent_sign < 0)
    {
      ...
      else if (ODLcurrent_value->item.type == TV_NULL_VALUE)
        {
          current_units = ODLcurrent_value->item.value.null_data.units;
        }
      else
