/*****************************************************************************
  ...
  Change: DWS Added code for NULL_CONSTANT values 0000-000 and 
              0000-00-00 to ODLExtractDate                             02/24/05
     
	      MDC Modified ODLConvertInteger. See notes in routine.        12/12/05
...
/* 12-12-05 MDC - Added to get values of LONG_MAX and LONG_MIN */
#ifdef LV_TOOL
#include <limits.h>
#endif
...
/*****************************************************************************
  ...
  Change History:
      12-12-05  MDC     Make warning message more descriptive to the user 
*****************************************************************************/
...

{
  ...
  if (errno == ERANGE)
    {
		/* 12-12-05 MDC - Make the warning msg. more descriptive by
		   stating what the number is being set to. */
#ifdef LV_TOOL

     sprintf (warning,
               "Magnitude of integer number is too large.  %s %s value of %ld.",
               "The number has been set to the maximum",
               (item.value.integer.number < 0 ? "negative" : "positive"),
			   (item.value.integer.number < 0 ? LONG_MIN : LONG_MAX));
#else
      sprintf (warning,
               "Magnitude of integer number is too large.  %s %s value.",
               "The number has been set to the maximum",
               (item.value.integer.number < 0 ? "negative" : "positive"));
#endif
...

{
  ...
  sp = strchr (vtext, '.');
  item.precision = (short) strcspn (++sp, "Ee");
  ...
  if (errno == ERANGE)
    {
      sprintf (warning,
               "Magnitude of real number is too large.  %s %s value.",
               "The number has been set to the maximum",
...
/*****************************************************************************
  ...
  Routine: ODLConvertNull
 
  Description: Save the Null string value and save any associated units 

  Input:  
          vtext   - Pointer to string containing the null string as ASCII text.
          vlength - Number of characters in value string.
...
VALUE_DATA  ODLConvertNull (vtext,vlength)
...

{
  ...
  char        warning[120];         /* Warning message text                 */
  char       *sp;                   /* Pointer to substring within input    */
  char       *ntext;                /* pointer to hold the null string value */
  int i;


  /* Set the value type and length and initialize the validity indicator */

  item.type      = TV_NULL_VALUE;
  item.length    = vlength;
  ...
  item.value.null_data.string = ntext;
  item.value.null_data.units  = NULL;
...

{
  ...
  long   day = 0;              /* Day of month                                  */
  long   doy = 0;              /* Day of year                                   */
  long   month = 0;            /* Month number                                  */
  long   year = 0;             /* Year number                                   */
 ...
 /* added test for NULL_CONSTANT = 0000-00 or 0000-00-00   DWS 02/24/05*/
  if(nparts == 2) 
  {
	  if ((year == 0) && (day == 0))
	  {
...

{
  ...
  long    hours = 0;           /* Hours, from input                             */
  long    minutes = 0;         /* Minutes, from input                           */
  double  seconds = 0;         /* Seconds, from input                           */
  ...
  else
    {
      ...
      if (seconds >= 60.0)
      ...
      else
        {
          ...
          if (sp != NULL)
            {
              item->precision = (short) strcspn (++sp, "Z+-");
            }
        }
    }
