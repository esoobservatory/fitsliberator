/*****************************************************************************
  ...
  Version 2.4 - 25 March 1993 - M. DeMore, Jet Propulsion Laboratory.
    Added #ifdef in ODLFormatTime to fix bug in zoned times.

  Version 2.5 - 09 May 2005 - M. Cayanan, Jet Propulsion Laboratory.
    Made change to ODLFormatString. See notes in routine.
...
/*****************************************************************************
  ...
  Routine: ODLFormatNullValue
 
  Description:  Formats a Null Value, with or without delimiters.
  ...
  Output:  The Null Vaule is placed into the statement string and the number
           of characters put in the string is returned as the function
           value.
...
int ODLFormatNullValue (stmt,item)
  ...
  if (item->valid == 0)
  ...
  else
    {
      ...
      if (hours == 0)
        ...
        {
#ifdef MSDOS_TC
	  sprintf (&stmt[len], "%+02d", hours);
#else
	  sprintf (&stmt[len], "%+03d", hours);
#endif
          len += 3;
...
/*****************************************************************************
 ...
 Change History:
     05-09-05   MDC   Made changes to correctly indicate newline and tab
	                  characters
 ****************************************************************************/
...

{
  ...
  long int     len;                  /* Number of characters in output string    */
      ...
      if (!is_pointer && c == '\\')
      {
         ...
         switch (c)
         {
            ...
            /* We encountered an explicit newline: go put out the line */
				 /* 05-09-05 MDC - We don't have to do anything in this case
				    other than to set the endline flag ON. The endline condition
					few lines below places a newline for us.
/*
            if (format_flag)
              {
                stmt[len++] = '\\';
...
/*
                ...
                l += 2;
*/
				/* 05-09-05 MDC - Tabs are more than 2 character lengths long, so
				we reflect that when we increment "l". Also, this is how you correctly
				state that we want to place a tab character in the file.
				*/
			    stmt[len++] = '\t';
				l += TAB_SIZE;
            }
		    ...
		    default:
				/* 05-09-05 MDC - Changed code to correctly indicate the correct escape
				   sequence character encountered */
/*
               stmt[len++] = '\\';
               stmt[len++] = c;
			   l+=2;
*/
			   sprintf(&stmt[len], "\\%c", c);
			   len++;
               l+=1;
               break;
         }
